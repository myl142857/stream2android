...
public class Device implements org.cybergarage.http.HTTPRequestListener, SearchListener
{
	...
	{
		System.out.println("[Cybergarage] Device.setURLBase" + value);
	...
	{
		System.out.println("[Cybergarage] updateURLBase" + host);
	...
	public void httpRequestRecieved(HTTPRequest httpReq)
	{
		if (Debug.isOn() == true)
			httpReq.print();
		System.out.println("[Cybergarage] Device.httpRequestRecieved host=" + httpReq.getSocket().getSocket().getRemoteSocketAddress().toString());
	
		if (httpReq.isGetRequest() == true || httpReq.isHeadRequest() == true) {
			System.out.println("[Cybergarage] Device.httpRequestRecieved GetRequest uri=" + httpReq.getURI());
			httpGetRequestRecieved(httpReq);
			return;
		}
		if (httpReq.isPostRequest() == true) {
			System.out.println("[Cybergarage] Device.httpRequestRecieved PostRequest uri=" + httpReq.getURI());
		...
		if (httpReq.isSubscribeRequest() == true || httpReq.isUnsubscribeRequest() == true) {
			if (httpReq.isSubscribeRequest() == true)
				System.out.println("[Cybergarage] Device.httpRequestRecieved SubscribeRequest uri=" + httpReq.getURI());
			else
				System.out.println("[Cybergarage] Device.httpRequestRecieved UnsubscribeRequest uri=" + httpReq.getURI());
	...
	{
		System.out.println("[Cybergarage] getDescriptionData host=" + host);
	...
	private void httpGetRequestRecieved(HTTPRequest httpReq)
	{
		...
		if (isDescriptionURI(uri) == true) {
			...
			System.out.println("[Cybergarage] httpGetRequestRecieved HostInterface.getInterface()" + localAddr);
			fileByte = getDescriptionData(localAddr);
		}
		else if ((embDev = getDeviceByDescriptionURI(uri)) != null) {
			String localAddr = httpReq.getLocalAddress();
			System.out.println("[Cybergarage] httpGetRequestRecieved httpReq.getLocalAddress()=" + localAddr);
			fileByte = embDev.getDescriptionData(localAddr);
		}
		else if ((embService = getServiceBySCPDURL(uri)) != null) {
			System.out.println("[Cybergarage] httpGetRequestRecieved getSCPDData()");
			...
			{
				System.out.println("[Cybergarage] httpGetRequestRecieved file exists:" + docFile.getAbsolutePath());
				fileByte = FileUtil.load( docFile );
			}
			else
			{
				System.out.println("[Cybergarage] httpGetRequestRecieved returnBadRequest() uri=" + uri);
				httpReq.returnBadRequest();
				return;
			}
		}
